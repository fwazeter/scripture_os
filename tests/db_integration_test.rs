...
async fn test_end_to_end_routing_and_fetching() {
    ...
    let texts = content::fetch_text(&pool, &resolved_path).await.unwrap();
...
async fn test_bible_vs_tanakh_psalm_shift() {
    ...
    let tanakh_v1 = content::fetch_text(&pool, "tanakh.ketuvim.psalms.51.1").await.unwrap();
    ...
    let bible_v1 = content::fetch_text(&pool, "bible.ot.psalms.51.1").await.unwrap();
...
async fn test_quran_hafs_vs_warsh_shift() {
    ...
    let warsh_v1 = content::fetch_text(&pool, "warsh.sura.1.1").await.unwrap();
...
async fn test_rigveda_overlapping_hierarchies() {
    ...
    let ashtaka_texts = content::fetch_text(&pool, "rigveda.ashtaka.1.adhyaya.1.varga.1.mantra.1").await.unwrap();
...
async fn test_traversal_adjacency_across_shifts() {
    ...
    let adjacency = traversal::get_adjacent_nodes(&pool, hafs_1_1_id).await.unwrap();
